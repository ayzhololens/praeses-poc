...
namespace HoloToolkit.Unity
{
    ...
    /// </summary>
    public partial class CursorManagerMagnetic : Singleton<CursorManager>
    {
        ...
        public Transform[] magnetChildren;
        ...
        private void Awake()
        {
            ...
            if (GazeManagerMagnetic.Instance == null)
            {
        ...
        private void LateUpdate()
        {
            ...
            if (CursorOnHolograms != null)
            {
                CursorOnHolograms.SetActive(GazeManagerMagnetic.Instance.Hit);
            }
            if (CursorOffHolograms != null)
            {
                CursorOffHolograms.SetActive(!GazeManagerMagnetic.Instance.Hit);
            }

            // Place the cursor at the calculated position.
            gameObject.transform.position = GazeManagerMagnetic.Instance.Position + GazeManagerMagnetic.Instance.Normal * DistanceFromCollision;

            // Orient the cursor to match the surface being gazed at.
            gameObject.transform.up = GazeManagerMagnetic.Instance.Normal;
        }

        public void magnetOffset()
        {
            magnetChildren = CursorOnHolograms.GetComponentsInChildren<Transform>();
            //magnetChildren[1].localPosition = new Vector3(.01f, 0, 0);
