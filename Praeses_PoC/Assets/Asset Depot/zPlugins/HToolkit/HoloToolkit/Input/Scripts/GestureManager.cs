...
namespace HoloToolkit.Unity
{
    ...
    public partial class GestureManager : Singleton<GestureManager>
    {
        ...
        private void OnTap()
        {
            ...
            {
                if (annotManager.videoReordingEnabled)
                {
                    annotManager.StartVideoRecording();
                }
                else if (annotManager.videoRecordingInProgress)
                {
                    annotManager.StopVideoRecording();
                }
                else if (annotManager.photoCaptureEnabled)
                {
                    annotManager.CapturePhoto();
                }
                else if (annotManager.tapToPlaceAnnotNode)
                {
                    annotManager.gameObject.GetComponent<annotationSpawner>().finishPlacingAnnotNode();
                }
                else if (annotManager.dictationInProgress)
                {
                    annotManager.StopDictation();
                }
            }
